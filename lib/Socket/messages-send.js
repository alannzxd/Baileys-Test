...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // PATCH: fix No sessions / No sender key di grup
...
while (attempt < 3) {
    try {
        ...
        break // sukses kirim, keluar loop
    } catch (err) {
        if (isGroup && (err.message.includes('No sessions') || err.message.includes('No sender key'))) {
            console.log(`[PATCH] Attempt ${attempt + 1} → Requesting sender key for ${jid}...`)
