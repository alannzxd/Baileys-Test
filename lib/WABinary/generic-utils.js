...
const assertNodeErrorFree = (node) => {
    ...
    if (errNode) {
        ...
        // PATCH: cegah crash kalau request sender key gagal ygy
        if (code === 406) {
            console.warn(`[INFO PATCH] Sender key request ditolak (406): ${text}`)
            return
        }
        if (code === 500) {
            console.warn(`[INFO PATCH] Internal server error (500) saat minta key: ${text}`)
            return
        }

        throw new boom_1.Boom(text, { data: code });
    }
};
